...
{if (getMarkerPos _x distance _position < 1000) then {_numberOfRBs = _numberOfRBs + 1}} forEach controlsX;
...
_roads = _roads call BIS_Fnc_arrayShuffle;
{
    ...
    if (_posroad distance _position > 400) then {
        ...
        if (count _roadsCon > 0) then {
            _nearestRB = [controlsX,_posroad] call BIS_fnc_nearestPosition;
            if (getMarkerPos _nearestRB distance _posroad > 1000) then {
                _name = format ["control_%1", count controlsX];
                ...
                controlsX pushBackUnique _name;
