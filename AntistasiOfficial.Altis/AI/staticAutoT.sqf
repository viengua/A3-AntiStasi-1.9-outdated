private ["_LeaderX","_static","_grupo","_maxCargo"];
...
while {true} do
	{
	if ((!alive _LeaderX) or (!alive _truckX) or (!alive _boy) or (!alive _static) or (!someAmmo _static) or (!canMove _truckX) or (not(_grupo getVariable "staticAutoT"))) exitWith {};
