...
_weaponList = genWeapons;
_magList = genAmmo;
_itemList = genItems;
_opticsList = genOptics;
_launcherList = genLaunchers;
_mineList = genMines;
_missileList = genMissiles;
...
_fnc_gear = {
	...
	switch (_cat) do {
		...
		case "missles": {
			...
			for "_i" from 0 to _typeInt do {
				_classInt = 1 + (floor random _classRan);
				_missleList = [_cat] call _fnc_checkList;

				_item = selectRandom _missleList;
				_crate addMagazineCargoGlobal [_item, _classInt];
			};
		};
	}
